...
namespace attendance_api.Services
{
    ...
    {
        byte[] GenerateUserSummaryPdf(List<AttendanceSummaryDto> data, string userName, string role, DateTime? fromDate, DateTime? toDate);
        byte[] GenerateAdminSummaryPdf(List<AttendanceSummaryDto> data, DateTime? fromDate, DateTime? toDate, string roleLabel = "All Roles");
    }
    ...
    public class PdfService : IPdfService
    {
        ...
        // ✅ roleLabel parameter added with default "All Roles"
        public byte[] GenerateAdminSummaryPdf(List<AttendanceSummaryDto> data, DateTime? fromDate, DateTime? toDate, string roleLabel = "All Roles")
        {
            var document = Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    page.Header()
                        ...
                        .Column(column =>
                        {
                            ...
                            // ✅ Role filter shown in header
                            column.Item().PaddingTop(3).Row(row =>
                            {
                                row.RelativeItem().Text($"Role Filter: {roleLabel}").FontSize(10);

                                if (fromDate.HasValue && toDate.HasValue)
                                {
                                    row.RelativeItem().AlignRight()
                                        .Text($"Period: {fromDate.Value:dd-MMM-yyyy} to {toDate.Value:dd-MMM-yyyy}")
                                        .FontSize(10);
                                }
                            });
                        });
