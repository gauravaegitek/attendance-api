...
namespace attendance_api.Controllers
{
    ...
    public class PerformanceController : BaseController
    {
        ...
        //   Avg Working Hours → 30% weightage  (max 30 points, target = 8h)
        //   WFH ratio         → 10% weightage  (max 10 points)
        // Grade: A=85+ | B=70+ | C=50+ | D=below 50
        // ─────────────────────────────────────────────────────────────────
        private async Task<EmployeeScoreDto> CalculateScore(int userId, int month, int year)
        {
            var user = await _context.Users.FindAsync(userId)
            ...
            decimal attendanceScore = attendancePct * 0.60m;
            decimal hoursScore      = Math.Min(avgHours / 8m * 30m, 30m);
            decimal wfhScore        = totalWorkingDays > 0
                ? Math.Min((decimal)wfhDays / totalWorkingDays * 100m * 0.10m, 10m)
                : 0;
        ...
        {
            if (month < 1 || month > 12)
                return ApiBadRequest("Invalid month. Use 1-12");
            ...
            {
                var score = await CalculateScore(targetUserId, month, year);
                return ApiOk("Performance score calculated", score);
            }
            catch (Exception ex)
            {
            ...
            [FromQuery] int? year  = null)
        {
            ...
            var query = _context.PerformanceReviews.Where(r => r.UserId == userId);
            ...
            if (!reviews.Any())
                return ApiNotFound("No reviews found for this period");
            ...
            return ApiOk($"Your performance reviews — total {result.Count}", result);
        }
        ...
        public async Task<IActionResult> SubmitPerformanceReview([FromBody] PerformanceReviewRequestDto dto)
        {
            ...
            return ApiOk(responseMessage, new PerformanceReviewResponseDto
            {
                UserId          = dto.UserId,
                UserName        = user.UserName,
                Role            = user.Role,
            ...
            [FromQuery] string? department = null)
        {
            ...
            var users = await usersQuery.ToListAsync();
            if (!users.Any()) return ApiNotFound("No users found");

            var scoreList = new List<EmployeeScoreDto>();
            foreach (var u in users)
            {
                try { scoreList.Add(await CalculateScore(u.UserId, month, year)); }
                catch { /* skip */ }
            }
            ...
            return ApiOk("Rankings retrieved successfully", new
            {
                totalEmployees = scoreList.Count,
                rankings       = rankingsByDept
            });
        }
        ...
        public async Task<IActionResult> GetAllReviews([FromQuery] int month, [FromQuery] int year)
        {
            ...
            return ApiOk($"Reviews retrieved — total {result.Count}", result);
        }
    }
}