    ...
    public class LeaveController : BaseController
    {
        ...
        public async Task<ActionResult<ApiResponse<LeaveResponseDto>>> ApplyLeave([FromBody] ApplyLeaveDto dto)
        {
            try
            {
                ...
                var validTypes = new[] { "casual", "sick", "earned", "halfday", "unpaid" };
                if (!validTypes.Contains(dto.LeaveType.ToLower()))
                    return ApiBadRequest("Invalid leave type. Use: casual, sick, earned, halfday, unpaid");
                ...
                var user = await _context.Users.FindAsync(userId);
                return ApiOk("Leave applied successfully", MapLeaveToDto(leave, user!));
            }
            catch (Exception ex) { return ApiServerError("Failed to apply leave", ex); }
        }
            ...
            [FromQuery] int? year = null)
        {
            ...
            {
                var userId = GetCurrentUserId();
                var query = _context.Leaves.Include(l => l.User).Where(l => l.UserId == userId);

                if (!string.IsNullOrEmpty(status)) query = query.Where(l => l.Status.ToLower() == status.ToLower());
                if (year.HasValue)                  query = query.Where(l => l.FromDate.Year == year.Value);

                var leaves = await query.OrderByDescending(l => l.CreatedOn).ToListAsync();
                return ApiOk("Leaves retrieved", leaves.Select(l => MapLeaveToDto(l, l.User!)).ToList());
            }
            catch (Exception ex) { return ApiServerError("Failed to retrieve leaves", ex); }
        }
            ...
            [FromQuery] DateTime? toDate = null)
        {
            try
            {
                ...
                if (fromDate.HasValue)              query = query.Where(l => l.FromDate >= fromDate.Value.Date);
                if (toDate.HasValue)                query = query.Where(l => l.ToDate <= toDate.Value.Date);

                var leaves = await query.OrderByDescending(l => l.CreatedOn).ToListAsync();
                var result = leaves.Select(l => MapLeaveToDto(l, l.User!)).ToList();

                return ApiOk($"Total {result.Count} leaves found", result);
            }
            catch (Exception ex) { return ApiServerError("Failed to retrieve leaves", ex); }
        }
        ...
        public async Task<ActionResult<ApiResponse<LeaveResponseDto>>> LeaveAction(int leaveId, [FromBody] LeaveActionDto dto)
        {
            try
            {
                var role = GetCurrentUserRole();
                if (role != "admin" && role != "hr")
                    return ApiForbidden("Access denied. Only admin/hr can approve or reject.");
                ...
                var leave = await _context.Leaves.Include(l => l.User).FirstOrDefaultAsync(l => l.LeaveId == leaveId);
                if (leave == null) return ApiNotFound("Leave not found");

                if (leave.Status != "pending")
                    return ApiBadRequest($"Leave is already {leave.Status}");
                ...
                return ApiOk($"Leave {leave.Status} successfully", MapLeaveToDto(leave, leave.User!));
            }
            catch (Exception ex) { return ApiServerError("Failed to process leave action", ex); }
        }
        ...
        public async Task<ActionResult<ApiResponse<string>>> CancelLeave(int leaveId)
        {
            try
            {
                ...
                if (leave == null) return ApiNotFound("Leave not found");
                if (leave.UserId != userId) return ApiForbidden("You can only cancel your own leave");
                if (leave.Status != "pending") return ApiBadRequest($"Cannot cancel leave which is already {leave.Status}");

                _context.Leaves.Remove(leave);
                await _context.SaveChangesAsync();

                return ApiOk("Leave cancelled successfully", "cancelled");
            }
            catch (Exception ex) { return ApiServerError("Failed to cancel leave", ex); }
        }

        private LeaveResponseDto MapLeaveToDto(Leave leave, User user) => new()
        {
            LeaveId      = leave.LeaveId,
            UserId       = leave.UserId,
            UserName     = user.UserName,
